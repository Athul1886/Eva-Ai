...
export const ProviderDashboardPage: React.FC = () => {
  ...
  const handleStatusChange = (bookingId: string, newStatus: BookingStatus) => {
    ...
    if (success) {
      ...
      setStatusActionSuccess(`Booking request marked as ${newStatus}`);
    ...
    <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-300 w-full min-w-0 max-w-full">
      ...
      {statusActionSuccess && (
        <div className="fixed bottom-6 right-6 z-50 p-4 rounded-2xl bg-surface-container-high/95 backdrop-blur-xl border border-secondary/40 shadow-2xl text-secondary text-sm font-semibold flex items-center gap-3 animate-in slide-in-from-bottom duration-200 max-w-[calc(100vw-3rem)]">
          ...
          <span className="break-words">{statusActionSuccess}</span>
        </div>
      )}
          ...
          <div className="p-6 sm:p-12 rounded-3xl bg-surface-container-high/40 border border-surface-container-highest/60 text-center space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-on-surface-variant max-w-sm mx-auto">
                Inquiries submitted by clients through the Eva-Ai planner will appear here. You can also generate a demo inquiry to test the workflow.
              </p>
            </div>
            <div className="pt-2">
              <button
              ...
              >
                ...
                <span>Generate Test Booking Request</span>
              </button>
            </div>
          </div>
        ) : (
                  ...
                  <div className="flex items-start justify-between gap-2 min-w-0">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[11px] text-on-surface-variant mt-1 flex flex-wrap items-center gap-1.5">
                        ...
                        <span className="text-outline">• ID: <span className="font-mono text-[10px] break-all">{booking.bookingId.substring(0, 12)}...</span></span>
                      </div>
                    </div>
                      ...
                      {booking.status === 'PENDING' && (
                        <>
                          <button
                            ...
                            className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1 px-3 py-2 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 font-bold text-xs border border-emerald-500/40 transition-colors"
                          >
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center px-2.5 py-2 rounded-xl bg-surface-container hover:bg-error/20 text-on-surface-variant hover:text-error text-xs font-semibold border border-surface-container-highest transition-colors"
                          >
                            Decline
                          </button>
                        </>
